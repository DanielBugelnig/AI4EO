...
class UNet(nn.Module):
    def __init__(self, input_number, output_number):
        ...
        self.conv1 = nn.Conv2d(self.input_number, 64, kernel_size=3, padding=1) # 64x570x570
        self.conv2 = nn.Conv2d(64, 64, kernel_size=3, padding=1) # 64x568x568
        self.maxPool1 = nn.MaxPool2d(kernel_size=2, stride=2) # 64x284x284

        self.conv3 = nn.Conv2d(64, 128, kernel_size=3, padding=1) # 128x282x282
        self.conv4 = nn.Conv2d(128, 128, kernel_size=3, padding=1) # 128x280x280
        self.maxPool2 = nn.MaxPool2d(kernel_size=2, stride=2) # 128x140x140

        self.conv5 = nn.Conv2d(128,256, kernel_size=3, padding=1) # 256x138x138
        self.conv6 = nn.Conv2d(256, 256, kernel_size=3, padding=1) # 256x136x136
        self.maxPool3 = nn.MaxPool2d(kernel_size=2, stride=2) # 256x68x68

        self.conv7 = nn.Conv2d(256, 512, kernel_size=3, padding=1) # 512x66x66
        self.conv8 = nn.Conv2d(512, 512, kernel_size=3, padding=1) # 512x64x64
        self.maxPool4 = nn.MaxPool2d(kernel_size=2, stride=2) # 512x32x32

        self.conv9 = nn.Conv2d(512, 1024, kernel_size=3, padding=1) # 1024x30x30
        self.conv10 = nn.Conv2d(1024, 1024, kernel_size=3, padding=1) # 1024x28x28
        ...
        self.conv1b = nn.Conv2d(1024, 512, kernel_size=3, padding=1) # 512x54x54 other 512 features come from encoder site
        self.conv2b = nn.Conv2d(512, 512, kernel_size=3, padding=1) # 512x52x52

        self.upconv2 = nn.ConvTranspose2d(512, 256, 2, 2) # 512x104x104
        self.conv3b = nn.Conv2d(512, 256, 3, padding=1) #256x102x102
        self.conv4b = nn.Conv2d(256, 256, 3, padding=1) # 256x100x100

        self.upconv3 = nn.ConvTranspose2d(256, 128, 2,2,) #256x200x200
        self.conv5b = nn.Conv2d(256, 128, 3, padding=1) #128x198x198
        self.conv6b = nn.Conv2d(128, 128, 3, padding=1) #128x196x196

        self.upconv4 = nn.ConvTranspose2d(128, 64, 2, 2) #128x392x392
        self.conv7b = nn.Conv2d(128, 64, 3, padding=1) #64x390x390
        self.conv8b = nn.Conv2d(64,64,3, padding=1) # 64x388x388
...
model = UNet(1,1)
summary(model, input_size=(1, 1, 1000, 1000))  # Example input size
